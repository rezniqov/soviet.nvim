...
local Config = require("soviet.config")
...
function M.get_group(name)
  ---@type {get: soviet.HighlightsFn, url: string}
  return Util.mod("soviet.groups." .. name)
end

---@param colors table<string, any>
---@param opts SovietConfig
...
function M.setup(colors, opts)
  ...
  -- Compatibility with the integration switches used before the plugin
  -- registry was introduced. Modern `plugins` settings below take priority.
  local legacy = {
    telescope = { "telescope" },
    gitsigns = { "gitsigns" },
    completion = { "blink", "cmp" },
    trees = { "neo-tree", "nvim-tree", "oil" },
    which_key = { "which-key" },
    indent_blankline = { "indent-blankline" },
    lazy = { "lazy" },
    snacks = { "snacks" },
  }
  for option, targets in pairs(legacy) do
    local enabled = opts.integrations[option]
    if enabled ~= nil then
  ...
  if opts.integrations.mini ~= nil then
    for _, group in pairs(M.plugins) do
      if group:find("^mini_") then
        groups[group] = opts.integrations.mini or nil
      end
    end
  end
